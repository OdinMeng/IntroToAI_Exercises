...
def sentence3() -> Expr:
    ...
    return conjoin([
        PacmanAlive_1 % disjoin([PacmanAlive_0 & ~PacmanKilled_0, ~PacmanAlive_0 & PacmanBorn_0]),
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    model = findModel(~(premise >> conclusion)) # check if the negation is UNSAT
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for combo in itertools.combinations(literals, 2):
        L.append(~combo[0] | ~combo[1])
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(0, 50):
        ...
        if m:=findModel(PropSymbolExpr(pacman_str, xg, yg, time=t) & conjoin(KB)):
...
def foodLogicPlan(problem) -> List:
    ...
    def FoodSucc(x, y, t):
        return PropSymbolExpr(food_str, x, y, time=t) % (PropSymbolExpr(food_str, x,y, time=t-1) & ~PropSymbolExpr(pacman_str, x, y, time=t-1)) 
    ...
    for t in range(0, 50):
        ...
        if t!=0:
            ...
            for f in food:
                KB.append(FoodSucc(f[0], f[1], t))

        if m:=findModel(~disjoin([PropSymbolExpr(food_str, f[0], f[1], time=t) for f in food]) & conjoin(KB)):
            return extractActionSequence(m, actions)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        # add pacphysics, action and percept
        ...
        for x,y in non_outer_wall_coords:
            ...
            m = entails(conjoin(KB), PropSymbolExpr(pacman_str, x,y, time=t))
            m_neg = entails(conjoin(KB), ~PropSymbolExpr(pacman_str, x,y, time=t))
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x,y in non_outer_wall_coords:
            ...
            m = entails(conjoin(KB), PropSymbolExpr(wall_str, x,y))
            m_neg = entails(conjoin(KB), ~PropSymbolExpr(wall_str, x,y))
